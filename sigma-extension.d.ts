 ...
 * 2. Or downloading this file and placing it in their project
 * 3. Referencing it in tsconfig.json:
 *    {
 *      "compilerOptions": {
 *        "typeRoots": ["./types", "./node_modules/@types"]
 *      }
 *    }
 * 4. Or importing from GitHub raw URL (if your tooling supports it)
 *
 * @version 1.1.0
...
export type UIElementType = 'input' | 'select' | 'checkbox' | 'textarea' | 'button' | 'separator' | 'text' | 'image' | 'skeleton';
...
export interface SigmaExtensionAPI {
  ...
  fs: {
    ...
    scoped: {
      requestDirectoryAccess(options?: { permission?: 'read' | 'write' | 'readWrite';
        title?: string;
